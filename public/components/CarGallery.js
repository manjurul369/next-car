        ...
        <div>
            <div className='flex flex-row gap-5 overflow-x-auto'>
                {thumbImg.map((img, index) => (
                    <div key={index} className={`w-1/3 h-25 lg:h-35 relative rounded-lg bg-secondary flex justify-center items-center overflow-hidden ` + (currentImg === img ? 'border-3 border-primary' : 'border-3 border-transparent')} onClick={() => handleThumbnailClick(img)}>
                        <Image
                            src={pattern2}
                            alt="Pattern"
                            fill
                        ...
                        <Image
                            ...
                            className="select-none w-9/10 hover:scale-105 transition-transform duration-300 cursor-pointer z-1"
                        />
                    </div>
                ))}
            </div>
